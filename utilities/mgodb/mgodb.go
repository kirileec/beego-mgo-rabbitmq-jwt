...
func Startup(sessionID string) error {
	...
	log.Trace(sessionID, "Startup", "MongoDB : Hosts[%s]", beego.AppConfig.String("mongodb::url"))
	log.Trace(sessionID, "Startup", "MongoDB : Database[%s]", beego.AppConfig.String("mongodb::database"))
	log.Trace(sessionID, "Startup", "MongoDB : Username[%s]", beego.AppConfig.String("mongodb::username"))

	config.Database = beego.AppConfig.String("mongodb::database")
	config.Hosts = beego.AppConfig.String("mongodb::url")
	config.UserName = beego.AppConfig.String("mongodb::username")
	config.Password = beego.AppConfig.String("mongodb::password")
...
func Execute(sessionID string, mongoSession *mgo.Session, databaseName string, collectionName string, dbCall DBCall) error {
	...
	if collection == nil {
		...
		log.CompletedError(err, sessionID, "GetCollection")
		return err
	}
	...
	if err != nil {
		log.CompletedError(err, sessionID, "dbCall")
		return err
	}
